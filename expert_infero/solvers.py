from expert_infero.sentences import Implication, Sentence, Symbol, And, Or, Not


def backward_chaining(
    rules: list[Implication],
    queries: list[Sentence],
    symtable: dict[str, bool],
    desc: dict[str, str],
):
    explanation: list[str] = []

    def ask(question: Sentence):
        if not isinstance(question, Symbol):
    ...
    def solve(query):
        ...
        for rule in rules:
            if rule.consequent == query:
                explanation.append(
                    f"{rule.consequent.describe(desc)} é verdade se {rule.antecedent.describe(desc)} for verdade..."
                )
                if solve(rule.antecedent):
